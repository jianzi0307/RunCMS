    ...
    return {
        ...
        init: function () {
            ...
            $('.ajax-get').live('click',function(){
                var target;
                var that = this;
                var oTableId = $(this).attr('data-target');
                var oTable = $(oTableId).DataTable();
                //添加该行为选中状态或者取消状态
        ...
        //设置宽屏或者窄屏
        layoutOptionSetScreen : function (isBoxed, footerIsFixed) {
            var resetLayout = function () {
                $("body").
                    removeClass("page-boxed");
                $('.header > .navbar-inner > .container').removeClass("container").addClass("container-fluid");
         ...
         * 生成表格
         * @param id 表格ID
         * @param isSort 是否启用排序功能
         * @param sorts 字段的排序属性
         * @param isSvrSide 是否服务端分页
         * @param ajaxUrl 获取数据的url
         */
        wrapTable: function(id, isSort, sorts, isSvrSide,ajaxUrl ) {
            //默认前端分页
            isSvrSide = isSvrSide || false;
            ajaxUrl = ajaxUrl || '';
            var tableObj = {};
            tableObj.aaSorting = [[ 0, "desc" ]];
            tableObj.bSort = (isSort != undefined) && isSort;
            if (isSort != undefined) {
                var sts = [];
                for (i = 0; i < sorts.length; i++) {
                    if (isSort && sorts != undefined) {
                        sts.push({"bSortable": sorts[i]});
                    } else {
                        sts.push({"bSortable": false});
                    }
                }
                tableObj.aoColumns = sts;
            }
            tableObj.aLengthMenu = [
                [15, 30, 50,100, -1],
                [15, 30, 50,100, "全部显示"]
            ];
            tableObj.iDisplayLength = 15;
            tableObj.sDom = "<'row-fluid'<'span6'l><'span6'f>r>t<'row-fluid'<'span6'i><'span6'p>>";
            ...
            //tableObj.sScrollXInner = "110%";
            //tableObj.bScrollCollapse = true;
            //tableObj.bProcessing = true;
            tableObj.bServerSide = (isSvrSide != undefined) && isSvrSide;
            if (tableObj.bServerSide) {
                tableObj.sAjaxSource = (ajaxUrl != undefined) ? ajaxUrl : '';
            }
            //状态保存
            tableObj.bStateSave = true;

            $('#'+id).dataTable(tableObj);
            jQuery('#'+id+'_logs_wrapper .dataTables_filter input').addClass("m-wrap medium"); // modify table search input
            jQuery('#'+id+'_logs_wrapper .dataTables_length select').addClass("m-wrap small"); // modify table per page dropdown
        },
